...
df = pd.read_csv(home+"data/preprocessed/deflator_reshaped.csv")
...
for index, row in df.iterrows():
    ...
    if len(observations) < len(colYears) and len(observations) > 3:
        ...
        if len(vvf) > 0:
            ...
            first_less_than_lb = np.sum(vals[0:first_observation]<lowest_val) > 0
        ...
        if len(vvl) > 0:
            ...
            last_less_than_lb = np.sum(vals[last_observation+1::]<lowest_val) > 0
    ...
    if len(observations) == len(colYears):
        new_rows.append(new_row)
    

dff = pd.DataFrame(new_rows, columns=df.columns)    
dff.to_csv(home+"data/preprocessed/deflator_imputed.csv", index=False)
